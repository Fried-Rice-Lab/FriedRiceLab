...
def merge_bn(w, b, gamma, beta, mean, var, eps, before_conv=True):
    ...
    if before_conv:
        ...
        rep_w = torch.mm(torch.diag(scale), w.view(
            out_feats, -1)).view(w.shape)

    # Reparameterizing bias
    if before_conv:
        rep_b = torch.mm(torch.sum(w, dim=(2, 3)),
                         bn_bias.unsqueeze(1)).squeeze() + b
    else:
...
def bn_parameter(pretrain_state_dict, k, dst='bn1'):
    ...
    mean = pretrain_state_dict[k.replace(
        f'{src}.weight', f'{dst}.running_mean')]
...
if __name__ == '__main__':
    ...
    model = FMEN(upscale=4, num_in_ch=3, num_out_ch=3, task='lsr',
    ...
    for k, v in tqdm(rep_state_dict.items()):
        ...
        if 'rep_conv.weight' in k:
            ...
            merge_b0b1 = b0.view(1, -1, 1, 1) * \
                torch.ones(1, mid_feats, 3, 3).cuda()
            merge_b0b1 = F.conv2d(input=merge_b0b1, weight=k1, bias=b1)

            # third step: merge the remain 1x1 convolution
            merge_k0k1k2 = F.conv2d(input=merge_k0k1.permute(
                1, 0, 2, 3), weight=k2).permute(1, 0, 2, 3)
            merge_b0b1b2 = F.conv2d(
                input=merge_b0b1, weight=k2, bias=b2).view(-1)
        ...
        # merge BN
            ...
            gamma, beta, mean, var, eps = bn_parameter(
            ...
            gamma1, beta1, mean1, var1, eps1 = bn_parameter(
                pretrain_state_dict, k, dst='bn2')
            gamma2, beta2, mean2, var2, eps2 = bn_parameter(
                pretrain_state_dict, k, dst='bn3')
            rep_w, rep_b = merge_bn(
                w, b, gamma1, beta1, mean1, var1, eps1, before_conv=True)
            rep_w, rep_b = merge_bn(
                rep_w, rep_b, gamma2, beta2, mean2, var2, eps2, before_conv=False)
            ...
            raise NotImplementedError(
                '{} is not found in pretrain_state_dict.'.format(k))
