        ...
        for idx_set = 1:num_set
            ...
            for scale = scale_all
                ...
                for idx_ext = 1:length(ext)
                    filepaths = cat(1, filepaths, dir(fullfile(hr_path, dataset{idx_set}, 'HR', ['x', num2str(scale)], ext{idx_ext})));
                end
                ...
                for idx_im = 1:length(filepaths)
                    ...
                    name_SR = name_HR;
                    im_HR = imread(fullfile(hr_path, dataset{idx_set}, 'HR', ['x', num2str(scale)], name_HR));
                    im_SR = imread(fullfile('../results', methods{idx_method}, 'visualization', 'test', dataset{idx_set}, degradation, ['x', num2str(scale)], name_SR));
