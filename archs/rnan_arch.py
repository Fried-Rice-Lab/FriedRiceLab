...
# Official GitHub: https://github.com/yulunzhang/RNAN
# ------------------------------------------------------------
import math
...
class _ResGroup(nn.Module):
    def __init__(self, conv, n_feats, kernel_size, act, res_scale):
        ...
        modules_body = []
        modules_body.append(
            ResAttModuleDownUpPlus(conv, n_feats, kernel_size, bias=True, bn=False, act=nn.ReLU(True),
                                   res_scale=1))
...
class _NLResGroup(nn.Module):
    def __init__(self, conv, n_feats, kernel_size, act, res_scale):
        ...
        modules_body = []
        modules_body.append(
            NLResAttModuleDownUpPlus(conv, n_feats, kernel_size, bias=True, bn=False, act=nn.ReLU(True),
                                     res_scale=1))
        modules_body.append(conv(n_feats, n_feats, kernel_size))
        self.body = nn.Sequential(*modules_body)
...
class RNAN(nn.Module):
    ...
    def check_image_size(self, x: torch.Tensor) -> torch.Tensor:
        _, _, h, w = x.size()
        windows_num_h = math.ceil(h / 2)
        windows_num_w = math.ceil(w / 2)
        mod_pad_h = windows_num_h * 2 - h
        mod_pad_w = windows_num_w * 2 - w
    ...
    def forward(self, x):
        ...
        return res_main[:, :, :h * self.upscale, :w * self.upscale]
